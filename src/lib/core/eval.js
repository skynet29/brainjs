(function () {
	...
	function replace2(s, char, callback) {
		//console.log('replace', firstChar, lastChar, s)
		let d = s.indexOf(char)
		while (d != -1) {
			const f = s.indexOf(char, d + 1)
			if (f == -1) {
	...
	function evalSimple(data, s) {
		...
		let idx = 1
		s = replace2(s, "'", function (expr) {
			if (expr.startsWith('\\u')) {
				expr = String.fromCharCode(parseInt(expr.substr(2), 16))
			}
